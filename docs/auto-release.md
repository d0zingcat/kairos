...
6. **Product Changelog (Optional)**: If `OPENAI_API_KEY` secret exists, a follow-up job runs `bun run changelog:generate:product` and commits `src/data/product-changelog.{zh,en}.json` back to `main`
...
This command reads only the latest released section from `CHANGELOG.md`, generates one new user-facing entry, and prepends it to `src/data/product-changelog.{zh,en}.json`.
Historical entries in those JSON files are preserved as-is and are not regenerated.
You can optionally set `OPENAI_CHANGELOG_MODEL` (default `gpt-4o-mini`).
In GitHub Actions, configure repository secret `OPENAI_API_KEY` and optional repository variable `OPENAI_CHANGELOG_MODEL`.
